...
class Device(inventory.Device):

    class_keys = inventory.Device.class_keys + {  # I know, you hate me now.  Such an ugly code.
                        'netflowCollectorGroupId' : lambda arg: arg is None or arg > 0,
                        'preferredCollectorGroupId',
                        'enableNetflow',
                        'snmp.version',  # extrvted from advanced aributes values (v2,v3,ping)
     }
    __RO_attributes__ = frozenset(( # not used
        'id',
        'awsTestResult',
        'azureTestResult',*
        'autoVisualResult',
